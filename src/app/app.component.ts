...
import { CustomImage, theme, CustomFile } from './models/index';
...
export class AppComponent {
  ...
  lng = -6.17458380;
  lat = 57.50690079;
  markers: Array<typeof CustomFile> = [];
  ...
  async loaded(e, image) {
    const gps = await exifr.gps(e.target.src);
    if (gps) {
  ...
  private getBase64File = file => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        const newFile = { file, base64: reader.result};
